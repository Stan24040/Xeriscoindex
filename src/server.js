...
// Xeris network health + stats
app.get('/api/xeris/health', async (req, res) => {
  try {
    const alive = await xerisRpc.healthCheck();
    const stats = alive ? await xerisRpc.getNetworkStats() : null;
    res.json({ online: alive, stats, node: xerisRpc.NODE_HOST,
               rpcPort: xerisRpc.RPC_PORT, explorerPort: xerisRpc.EXPLORER_PORT });
  } catch(e) { res.json({ online: false, error: e.message }); }
});

// Xeris network economics dashboard
app.get('/api/xeris/economics', async (req, res) => {
  try {
    const econ = await xerisRpc.getNetworkEconomics();
    if (!econ) return res.status(503).json({ error: 'Xeris node unreachable' });
    res.json(econ);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// All registered tokens on Xeris chain
app.get('/api/xeris/tokens', async (req, res) => {
  try {
    const tokens = await xerisRpc.getRegisteredTokens();
    res.json({ tokens, count: tokens.length });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Active launchpad bonding curves
app.get('/api/xeris/launchpads', async (req, res) => {
  try {
    const pads = await xerisRpc.getLaunchpads();
    res.json({ launchpads: pads, count: pads.length });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Launchpad buy quote
app.get('/api/xeris/launchpad/:id/quote', async (req, res) => {
  const xrsAmount = parseInt(req.query.xrs_amount) || 1_000_000_000;
  try {
    const quote = await xerisRpc.getLaunchpadQuote(req.params.id, xrsAmount);
    if (!quote) return res.status(404).json({ error: 'Contract not found' });
    res.json(quote);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Swap contracts (AMM pools)
app.get('/api/xeris/pools', async (req, res) => {
  try {
    const pools = await xerisRpc.getSwapContracts();
    res.json({ pools, count: pools.length });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Recent blocks from Xeris chain
app.get('/api/xeris/blocks', async (req, res) => {
  const page = parseInt(req.query.page) || 1;
  const size = Math.min(50, parseInt(req.query.size) || 20);
  try {
    const blocks = await xerisRpc.getRecentBlocks(page, size);
    res.json({ blocks, page, size });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Recent transactions from Xeris chain
app.get('/api/xeris/transactions', async (req, res) => {
  const page = parseInt(req.query.page) || 1;
  const size = Math.min(50, parseInt(req.query.size) || 20);
  try {
    const txs = await xerisRpc.getRecentTransactions(page, size);
    res.json({ transactions: txs, page, size });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Wallet balance on Xeris chain (native XRS + all tokens)
app.get('/api/xeris/balance/:address', async (req, res) => {
  try {
    const lamports = await xerisRpc.getBalance(req.params.address);
    res.json({
      address:  req.params.address,
      lamports,
      xrs:      lamports / 1_000_000_000,
    });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Latest blockhash (needed for tx building)
app.get('/api/xeris/blockhash', async (req, res) => {
  try {
    const bh = await xerisRpc.getLatestBlockhash();
    if (!bh) return res.status(503).json({ error: 'Could not fetch blockhash' });
    res.json(bh);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Testnet airdrop proxy (rate-limited)
const airdropLimiter = rateLimit({
  windowMs: 15 * 1000, max: 1,
  message: { error: 'Airdrop rate limited — wait 15 seconds' },
});
app.get('/api/xeris/airdrop/:address/:amount', airdropLimiter, async (req, res) => {
  try {
    const xrsAmount = Math.min(10, parseFloat(req.params.amount) || 10);
    const result = await xerisRpc.requestAirdrop(req.params.address, xrsAmount);
    res.json(result);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
