...
function getStats() {
  const pairs  = getAll();
  const xeris  = pairs.find(p => p.symbol === 'XERIS');
  return {
...
async function syncFromDexScreener() {
  try {
    console.log('[DexScreener] Fetching XERIS pairs...');
    const pairs = await dexscreener.fetchXerisTokenPairs();
    ...
    console.log(`[DexScreener] ✓ Synced ${updates.length} XERIS pairs`);
  } catch (err) {
...
async function syncFromXerisRpc() {
  ...
  if (!alive) {
    console.warn('[XerisRPC] ⚠ Node not reachable — check XERIS_RPC_URL');
    return;
  }
  const stats = await xerisRpc.getNetworkStats();
  if (stats) console.log(`[XerisRPC] TPS: ${stats.tps} | Slot: ${stats.currentSlot}`);
  // TODO: parse XerisSwap pool accounts → price data once DEX program is deployed
}
...
async function startPriceEngine() {
  ...
  if (USE_XERIS_RPC) {
    ...
    setInterval(syncFromXerisRpc, 5000);
    return;
  }

  // Default: DexScreener live data (XERIS on Solana)
  await syncFromDexScreener();
  setInterval(syncFromDexScreener, 30_000);
